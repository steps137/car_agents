...
class CarModelNormal:
    ...
    engine_power = 5      # acceleration by pressing the gas pedal once
    steering_sensitivity = 100

    w_max = 30*np.pi/180  # maximum rotation angle     in rad
    v_max = 30            # 30 m/sec max velocity
...
class Car:
    def __init__(self, pos, dir, vel, index, kind=0):
        ...
        self.dir = dir             # car orientation vector (from the center of mass forward)
    ...
    def action(self, act, dt):
        df, dw = act[0], act[1]
        self.w11 += self.model.steering_sensitivity * dt*dw * np.pi/180
        self.w11 = max( min(self.w11, self.model.w_max), -self.model.w_max)
        self.w12 = self.w11         # трапеция!
        self.actions = act.copy()
        self.force = self.model.engine_power*df
    #---------------------------------------------------------------------------
    ...
    def phys(self, dt, eps=1e-8):
        ...
        a = force - self.model.mu1*v*self.vel - self.model.mu2*self.vel/(v+eps) \
